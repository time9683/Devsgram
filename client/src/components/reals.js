...
var obersevador = new IntersectionObserver((entradas,_)=>{        
  entradas.forEach(en => {  if(en.isIntersecting){   en.target.currentTime = 0; en.target.play();  }else{en.target.pause()}  })                                                                                                                                                                            
                     },{threshold:0.4})
...
<div className={ViewComents === false ? 'container' : 'container preview' }>
...
const Display = (props) => {
  ...
  if(loading){
   ...
   return  <div style={{height:'90vh',display:"flex",justifyContent:"center",alignItems:"center",backgroundColor:'black'}}> <div className="spinner"></div></div>
...
if(error){
 ...
 return  <div style={{height:'90vh',backgroundColor:"black",color:"white",display:"flex",justifyContent:"center",alignItems:"center"}}><p>hubo un error en el ingreso de datos</p></div>
...
return   data.GetReals.map(vide =>  <Video   ComentsCan={vide.coments}  status={false} key={vide._id} src={vide.src} likes={vide.likes} id={vide._id} handleCommnets={props.handleC}   />);
...
if(data){
  ...
  grayhear =  <img src={hearfillG} className='like-hear-front'  onAnimationEnd={splace} ></img>
...
if(!Melike){ 
  ...
  LikeTo({variables:{id:props.id,person:localStorage.getItem("ID_A")}})
  playLike(true)

}else{
  dislikeTo({variables:{id:props.id,person:localStorage.getItem("ID_A")}})
  playLike(false)
...
const commas = (likes) =>{
  ...
  return likes.toString().replace(/\B(?=(\d{3})+(?!\d))/g,',')
    ...
    <img src={Melike == true ? hearfill : hear} alt='likes icon' className='hear'/>
    <p style={{color:"white"}} >{commas(props.likes.length)}</p>
    </div> 
    
   
      <div className="atributo"  onClick={()=>{ props.handleCommnets(props.id) }}>  
    <img src={burble} className="hear" alt='comments icon'  />
    <p style={{color:"white"}}>{props.ComentsCan ? commas(props.ComentsCan.length) : 0}</p>
    </div> 
  ...
  {grayhear}
     ...
     <video id="video" loop={true} className="video"   onDoubleClick={like}  src={props.src} onClick={plays} ref={Ev}/>
...
<li ><img src={flechita} alt='volver' style={{width:'30px',transform:'rotate(180deg)',marginRight:'30px'}} onClick={()=>props.handleCommnets()}/> 
 <span style={{fontSize:'20px',fontWeight:'bold'}}>Comentarios</span></li>
<li><img src={send}  alt='send'/></li>
</ul>

</div>

<div className="DisplayComents">
{coments}
...
<img src={props.img} alt='imagen usuario'></img>
<p>{props.name}</p>
<p>{props.text}</p>
</div>
